...
class LandingWidePricingTest extends TestCase
{
    ...
    public function test_wide_pricing_renders_four_sections_with_configurable_options(): void
    {
        ...
        $this->assertStringContainsString('одно рабочее место', $html);
        $this->assertStringContainsString('двадцать рабочих мест', $html);
        $this->assertStringNotContainsString('за одно рабочее место', $html);
        $this->assertStringNotContainsString('за двадцать рабочих мест', $html);
    ...
    public function test_cleanup_migration_removes_only_the_unused_note_settings(): void
    {
        $plan = LandingBlock::query()->where('section_slug', 'pricing_wide')->where('block_type', 'plan')->firstOrFail();
        $currencySuffix = $plan->extra['currency_suffix'];
        $plan->update([
            'description' => 'за {users} {workplaces}',
            'extra' => array_merge($plan->extra, [
                ...
                'month_label' => 'Месяц',
                'year_label' => 'Год',
                'users_decrease_label' => 'Уменьшить',
                'users_increase_label' => 'Увеличить',
            ]),
        ]);

        $migration = require database_path('migrations/2026_09_23_130000_remove_unused_wide_pricing_fields.php');
        $migration->up();

        $plan->refresh();
        $this->assertSame('за одного пользователя в месяц', $plan->description);
        $this->assertSame($currencySuffix, $plan->extra['currency_suffix']);
        foreach (['workplace_one', 'workplace_few', 'workplace_many', 'period_label', 'month_label', 'year_label', 'users_decrease_label', 'users_increase_label'] as $key) {
            $this->assertArrayNotHasKey($key, $plan->extra);
        }

        $plan->update(['description' => 'Моя подпись']);
        $migration->up();
        $this->assertSame('Моя подпись', $plan->fresh()->description);
    }
}
