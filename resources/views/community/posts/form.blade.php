...
<div class="landing-shell community-form-shell">
    ...
    <div class="community-form-card">
        ...
        <form method="POST" action="{{ $post->exists ? route('community.posts.update', $post) : route('community.posts.store') }}" class="community-form" enctype="multipart/form-data">
            ...
            <fieldset><legend>Текст или ссылка</legend>
                <label>
                    Текст публикации
                    <textarea name="body_markdown" rows="12" maxlength="20000" placeholder="Опишите вопрос или поделитесь опытом…">{{ old('body_markdown', $post->body_markdown) }}</textarea>
                    <small>Можно использовать абзацы, списки и ссылки. Если добавляете фото, текст необязателен.</small>
                </label>
            ...
            @include('community.photos._editor', ['existingPhotos' => $post->exists ? $post->photos : collect()])
