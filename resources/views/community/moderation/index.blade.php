...
<div class="landing-shell community-form-shell">
    ...
    <div class="community-report-list">
        @forelse($reports as $report)
            <article class="community-report-card"><div><strong>{{ $report->target_type }} #{{ $report->target_id }}</strong><span>{{ $report->reason }}</span><p>{{ $report->details }}</p><time>{{ $report->created_at->diffForHumans() }}</time></div>
