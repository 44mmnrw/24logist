...
class MaxCommunityAuthController extends Controller
{
    ...
    public function approve(
        ...
        MaxInitDataReplayGuard $replayGuard,
    ): JsonResponse {
        abort_unless($this->siteSettings->communityMaxEnabled(), 404);
        ...
        $challenge = DB::transaction(function () use ($data, $maxData, $identities, $replayGuard): CommunityLoginChallenge {
    ...
    public function session(
        ...
        [$user, $challenge] = DB::transaction(function () use ($maxData, $linkTo, $identities, $replayGuard): array {
            $replayGuard->consume($maxData);
            $user = $identities->resolve('max', (string) $maxData['user']['id'], $linkTo, true);
            $user->identities()->where('provider', 'max')->update(['bot_status' => 'active']);
