...
final class CommunityAiPersonaPromptBuilder
{
    ...
    public function buildFromValues(
        ...
        $errorChance = max(0, min(60, (int) data_get($settings, 'literacy_profile.error_chance', 5)));
        $imperfections = $this->value(
            $settings,
            'literacy_profile.imperfections',
            'редкий пропуск запятой или одна простая опечатка',
        );
...
Примерно в {$errorChance}% сообщений допускай не более одной естественной ошибки: {$imperfections}. Процент относится к сообщениям, а не к словам. В остальных сообщениях не добавляй намеренных ошибок.
...
13. В значениях title и body не используй длинное тире, двоеточия и точки с запятой. Если нужна пауза, поставь точку. Обычный дефис внутри слова используй по правилам русского языка.
PROMPT;
