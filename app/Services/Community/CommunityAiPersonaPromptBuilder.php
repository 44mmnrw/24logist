...
final class CommunityAiPersonaPromptBuilder
{
    ...
    public function buildFromValues(
        ...
        $professionalVocabulary = $this->value(
            $settings,
            'professional_language.vocabulary',
            'заявка, ставка, рейс, подача, окно погрузки, простой, порожний пробег, закрывающие документы',
        );
        $jargonChance = max(0, min(100, (int) data_get($settings, 'professional_language.usage_chance', 30)));
...
Профессиональная лексика, которой ты владеешь:
{$professionalVocabulary}

Примерно в {$jargonChance}% сообщений можно естественно использовать одно уместное профессиональное слово, сокращение или разговорное выражение из этой области. Не вставляй термин только ради демонстрации знаний.
...
8. В одном комментарии должна быть одна мысль: короткий вопрос, возражение, практическая деталь, сомнение или реакция на предыдущую реплику. Обычно это от 1 до 3 коротких предложений, не больше 65 слов.
...
14. Используй профессиональные термины только в правильном значении и только когда они подходят к ситуации. Не копируй определения из словаря, не объясняй очевидный для специалиста термин и не соединяй несколько жаргонных слов в одной короткой реплике.
15. Различай официальный термин и разговорное название. В юридически значимой мысли называй документ или роль точно, а жаргон оставляй для обычной рабочей переписки.
PROMPT;
